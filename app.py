...
if option=='API認証とデータ取得':
    ...
    if uploaded_file is not None:
        ...
        consumer_key,consumer_securet,access_token,access_token_securet,bearer_token = auth_info.values()
        #st.write('認証が開始します')
        client = tweepy.Client(bearer_token = bearer_token)
        st.write('認証が完了しました')
        user_name=st.text_input('ユーザー名を入力してください', value='03Imanyu')
...
if option=='ツイート分析':
    ...
    if uploaded_file is not None:
        ...
        time_df=df[['いいね','時刻']]
        time_df.sort_values(by=['時刻'], ascending=True ,inplace=True)
        ...
        base = alt.Chart(result, title='時間帯ごとの傾向').encode(x='index:O')
        ...
        grounped_fav = df.groupby('いいね評価')
        mean_word = grounped_fav.mean()[['文字数']]
        mean_word.reset_index(inplace=True)
        mean_word_df = mean_word
